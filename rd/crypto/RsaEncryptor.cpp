...
namespace crypto
{
	RsaEncryptor::RsaEncryptor() : _pub(0), _priv(0)
	{}

	RsaEncryptor::RsaEncryptor(const RsaEncryptor&)
	{
	...
	void RsaEncryptor::genRsa(int bits)
	{
		...
		_priv = RSAPrivateKey_dup(pRSA);
		_pub = RSAPublicKey_dup(pRSA);
		RSA_free(pRSA);
		BN_free(bn);
	}
	...
	{
		return RSA_size(_pub);
	}

	int RsaEncryptor::privKeySize()
	{
		return RSA_size(_priv);
	}
	...
	int RsaEncryptor::encrypt(const uint8_t* in, int size, uint8_t* out)
	{
		...
		int res = encrypt_RSA(_pub, _inBuff.data(), size, _outBuff.data());
		if (res > 0)
		{
	...
	int RsaEncryptor::decrypt(const uint8_t* in, int size, uint8_t* out)
	{
		...
		int res = decrypt_RSA(_priv, _inBuff.data(), _outBuff.data());
		if (res > 0)
		{
			memcpy(out, _outBuff.data(), res);
		}
	...
	std::string RsaEncryptor::privKey()
	{
		...
		PEM_write_bio_RSAPrivateKey(bio, _priv, NULL, NULL, 0, NULL, NULL);
	...
	{
		BIO* bio = BIO_new(BIO_s_mem());
		int res = PEM_write_bio_RSA_PUBKEY(bio, _pub);
		const int keylen = BIO_pending(bio);
		std::string key;
		key.resize(keylen);
		BIO_read(bio, &key[0], keylen);
