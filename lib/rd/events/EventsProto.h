...
#ifdef _WIN32
#include "protoWin/serialize.pb.h"
#endif

#ifdef __linux__ 
#include "protolin/serialize.pb.h"
#endif
		...
		static bool encode(const T& in, std::vector<uint8_t>& res)
		{
			...
			if constexpr (std::is_same_v<T, Rd::FilesEvent>)
			{
				...
				if (in.type == Rd::FilesEvent::DIR_REQUEST)
					ev.mutable_dir_request()->set_path(in.path1.generic_string());
				...
				if (in.type == Rd::FilesEvent::DIR_RECURSION_REQUEST)
					ev.mutable_dir_recursion_request()->set_path(in.path1.string());
				...
				if (in.type == Rd::FilesEvent::REMOVE)
					ev.mutable_remove()->set_path(in.path1.string());

				if (in.type == Rd::FilesEvent::MAKE_DIR)
					ev.mutable_make_dir()->set_path(in.path1.string());

				if (in.type == Rd::FilesEvent::RENAME)
				{
					ev.mutable_rename()->set_src(in.path1.string());
					ev.mutable_rename()->set_dest(in.path2.string());
				}
			...
			if constexpr (std::is_same_v<T, Rd::TransferEvent>)
			{
				...
				if (auto pval = std::get_if<TransferEvent::Post>(&in.val))
				{
					...
					ev.mutable_post()->set_path(pval->path.string());
					if (pval->data.size())
				...
				if (auto pval = std::get_if<TransferEvent::Get>(&in.val))
				{
					...
					ev.mutable_get()->set_path(pval->path.string());
				}
