...
{
	//std::cout << "openScreen " <<_pid << std::endl;
	if (!_pid)
	{
...
{
	//std::cout << "closeScreen " << _pid << std::endl;
	if (_pid)
	{
		kill(_pid, SIGTERM);
		waitpid(_pid, 0, 0);
...
bool LinDisplay::getResolutions(std::string& res)
{
	...
	if (child == 0) {
		...
		std::string disp = "DISPLAY=:0.0";
		/*std::string dname = getenv("DISPLAY");
		if (dname.empty())
			disp += ":0.0";
		else
			disp += dname;*/
...
{
	//const char* comm = "scr";
	const char* comm = "Screen.out";
	//const char* comm = "/home/user/projects/Screen/bin/x64/Release/Screen.out";
//	signal(SIGCHLD, SIG_IGN);
	...
	if (pid > 0) {
		_pid = pid;
		//std::cout << "fork succ: " << pid << "\n";
	}

	if (pid == -1) {
		//std::cout << "fork err: " << pid << "\n";
	}
}
